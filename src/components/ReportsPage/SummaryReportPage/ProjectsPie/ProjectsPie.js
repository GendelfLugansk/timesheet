...
const ProjectsPie = memo(
  ({ logItems, isSyncing, responsiveLegend = true, width, height }) => {
      ...
      rawData = Object.values(
        rawData.reduce((acc, { project, durationHours }, index) => {
          ...
          const projectOrOther =
            isOther(durationHours) || index >= maxSectors
              ? "__other__"
              : project;
...
export default memo(withResizeDetector(ProjectsPie));
